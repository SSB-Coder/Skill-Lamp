...
- **Governed SQL Trace**:
  ...
  FROM campus_intelligence.gold.gold_fact_placement_history
...
- **Governed SQL Trace**:
  ...
  WITH exploded_skills AS (
    ...
    FROM campus_intelligence.gold.gold_dim_company_criteria
    LATERAL VIEW EXPLODE(SPLIT(COALESCE(mandatory_skills, ''), ',')) t AS skill
...
- **Governed SQL Trace**:
  ```sql
  SELECT 
    CASE WHEN sk.is_certified = TRUE THEN 'Certified AI Skill' ELSE 'Non-Certified / Standard' END AS certification_cohort,
  ...
  GROUP BY CASE WHEN sk.is_certified = TRUE THEN 'Certified AI Skill' ELSE 'Non-Certified / Standard' END
...
- **Governed SQL Trace**:
  ...
  FROM campus_intelligence.gold.gold_dim_students s
  JOIN campus_intelligence.gold.gold_fact_student_skills sk ON s.student_id = sk.student_id
  LEFT JOIN campus_intelligence.gold.gold_fact_placement_history ph ON s.student_id = ph.student_id
